...
export function StatisticsPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-[22px] font-semibold leading-tight text-foreground">Statistics</h1>
        <p className="text-[13px] text-muted-foreground mt-1">Summary, trend, and account balances.</p>
      </div>
      {error && <p className="text-[12px] text-danger">{error}</p>}
      <div className="grid gap-4 md:grid-cols-3">
        {[
          { label: 'Total income', value: fmtMoney(data?.totalIncome ?? '-') },
          { label: 'Total expense', value: fmtMoney(data?.totalExpense ?? '-') },
          { label: 'Net', value: fmtMoney(data?.net ?? '-') },
        ].map((c) => (
      ...
      <Card className="overflow-hidden">
        <div className="px-6 py-3 border-b border-border bg-muted text-[11px] font-medium text-muted-foreground">Account balances</div>
